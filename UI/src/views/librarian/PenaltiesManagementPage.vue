        ...
        <div @click.stop class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          <form @submit.prevent="savePenalty">
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                  <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <div class="font-medium">{{ selectedPenalty?.user.name }}</div>
                    </div>
                    <div>
                      <span class="text-gray-500">Borrowing:</span>
                      <div class="font-medium">#{{ selectedPenalty?.borrowing.id }}</div>
                    </div>
                    <div>
            ...
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
              ...
              >
                {{ isSaving ? 'Saving...' : 'Update Penalty' }}
              </button>
              <button
                @click="closeModal"
                type="button"
    ...
    <div v-if="showDetailsModal" class="fixed inset-0 z-50 overflow-y-auto" @click="closeDetailsModal">
      <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div @click.stop class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
              ...
              <div class="bg-blue-50 rounded-lg p-4">
                ...
                <div class="space-y-2">
                  <div class="flex justify-between">
                    ...
                    <span class="text-sm font-medium text-blue-600">#{{ selectedPenalty.borrowing.id }}</span>
                  </div>
                  <div class="flex justify-between">
                  ...
                  <div class="flex justify-between">
                    ...
                    <span class="text-sm font-medium">{{ formatDate(selectedPenalty.borrowing.dueAt) }}</span>
                  </div>
                  <div class="flex justify-between">
                  ...
                  <div class="mt-3">
                    ...
                    <div class="mt-1 space-y-1">
                      <div v-for="book in selectedPenalty.borrowing.books" :key="book.id" class="text-sm">
          ...
          <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <button
              v-if="!selectedPenalty?.isPaid"
              @click="markAsPaid(selectedPenalty)"
...
import { getAllPenalties, getBorrowingById } from '@/api'
...
const filteredPenalties = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter(penalty =>
      ...
      penalty.borrowing.id.toString().includes(query)
    )
  }
...
const editPenalty = (penalty) => {
  ...
  penaltyForm.isPaid = penalty.isPaid
  
  showEditModal.value = true
}

const markAsPaid = async (penalty) => {
  try {
    // Add your mark as paid API call here
    // await markPenaltyAsPaid(penalty.id)
    
    // Mock: Update penalty status
    const index = penalties.value.findIndex(p => p.id === penalty.id)
    if (index !== -1) {
      penalties.value[index].isPaid = true
    }
    
    message.value = 'Penalty marked as paid successfully'
    setTimeout(() => message.value = '', 3000)
    closeDetailsModal()
    
  } catch (error) {
    console.error('Error marking penalty as paid:', error)
    errorMessage.value = 'Failed to mark penalty as paid'
    setTimeout(() => errorMessage.value = '', 3000)
  }
}
...
const savePenalty = async () => {
  try {
    ...
    // Add your update penalty API call here
    // await updatePenalty(penaltyForm.id, {
    //   description: penaltyForm.description,
    //   isPaid: penaltyForm.isPaid
    // })
    
    // Mock: Update in penalties array
