  ...
  <div class="min-h-screen bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 py-6">
      ...
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Library Statistics</h1>
        ...
        <div class="flex justify-between items-center mb-4">
          ...
          <div class="flex space-x-2">
            <button
              ...
              class="px-2 py-1 text-sm bg-purple-100 text-purple-700 rounded hover:bg-purple-200 disabled:opacity-50"
            >
        ...
        <div v-else>
          <div class="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
            ...
            <div class="bg-purple-50 p-3 rounded">
              ...
              <p class="text-xl font-bold text-purple-600">{{ collectionStats.data.availabilityRate.toFixed(1) }}%</p>
            </div>
            <div class="bg-teal-50 p-3 rounded">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            ...
            <div class="flex items-end">
              <button
                ...
                class="w-full p-2 bg-purple-600 text-white rounded hover:bg-purple-700 disabled:opacity-50"
              >
                Apply Filter
              </button>
            </div>
          ...
          <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow mb-6">
            <thead>
              <tr class="border-b bg-gray-50">
                ...
                <th class="text-left py-2 px-3 text-gray-700">Borrows</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(book, index) in collectionStats.data.popularBooks" :key="book.id" class="border-b hover:bg-gray-50">
                ...
                <td class="py-2 px-3">{{ book.borrowCount }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="bg-white rounded-lg shadow mb-6 p-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          ...
          <div class="flex items-end">
            <button
              ...
              class="w-full p-2 bg-green-600 text-white rounded hover:bg-green-700 disabled:opacity-50"
            >
              Apply Filter
            </button>
          </div>
        ...
        <div v-else>
          ...
          <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow">
            <thead>
              <tr class="border-b bg-gray-50">
                <th class="text-left py-2 px-3 text-gray-700">User ID</th>
                <th class="text-left py-2 px-3 text-gray-700">Type</th>
                <th class="text-left py-2 px-3 text-gray-700">Borrows</th>
                <th class="text-left py-2 px-3 text-gray-700">Visits</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="user in userStats.data.topActiveUsers" :key="user.userId" class="border-b hover:bg-gray-50">
                ...
                <td class="py-2 px-3">{{ user.userType }}</td>
                <td class="py-2 px-3">{{ user.borrowCount }}</td>
                <td class="py-2 px-3">{{ user.visitCount }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="flex justify-between items-center mb-4">
          ...
          <div class="flex space-x-2">
            <button
              ...
              class="px-2 py-1 text-sm bg-red-100 text-red-700 rounded hover:bg-red-200 disabled:opacity-50"
            >
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          ...
          <div class="flex items-end">
            <button
              ...
              class="w-full p-2 bg-red-600 text-white rounded hover:bg-red-700 disabled:opacity-50"
            >
              Apply Filter
            </button>
          </div>
        ...
        <div v-else>
          <div class="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
            ...
            <div class="bg-purple-50 p-3 rounded">
              ...
              <p class="text-xl font-bold text-purple-600">{{ formatNumber(financialStats.data.outstandingCount) }}</p>
            </div>
            <div class="bg-teal-50 p-3 rounded">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
            <div class="bg-gray-100 p-3 rounded">
              <p class="text-sm text-gray-500">Late Return Fees</p>
              <p class="text-lg font-bold text-gray-800">${{ formatNumber(financialStats.data.penaltyBreakdown.lateReturnFees) }}</p>
            </div>
            <div class="bg-gray-100 p-3 rounded">
            ...
            <div class="bg-gray-100 p-3 rounded">
              ...
              <p class="text-lg font-bold text-gray-800">${{ formatNumber(financialStats.data.penaltyBreakdown.otherFees) }}</p>
            </div>
          </div>
...
import { getBorrowingStatistics, getBookStatistics } from '@/api'

const getUserActivityStatistics = async (params) => {
  await new Promise(resolve => setTimeout(resolve, 500))
  const activeUsers = 850;
  const totalUsers = 1500;
  const newRegistrations = 45;
  const libraryVisits = 3200;
  return {
    success: true,
    data: {
      ...
      avgBorrowsPerUser: 2.8,
      avgVisitsPerUser: libraryVisits / activeUsers,
      userDemographics: {
        students: 1000,
        faculty: 300,
        staff: 150,
      ...
      topActiveUsers: [
        { userId: "user_A1B2", userType: "Student", borrowCount: 25, visitCount: 10 },
        { userId: "user_C3D4", userType: "Faculty", borrowCount: 20, visitCount: 8 },
        { userId: "user_E5F6", userType: "Student", borrowCount: 18, visitCount: 12 }
      ],
      activityTrends: [
        { date: "2024-06-01", activeUsers: 120, newRegistrations: 5, libraryVisits: 200 },
        { date: "2024-06-02", activeUsers: 130, newRegistrations: 3, libraryVisits: 210 }
      ],
...
const getFinancialStatistics = async (params) => {
  await new Promise(resolve => setTimeout(resolve, 500))
  const totalPenalties = 2500;
  const collectedPenalties = 1800;
  const outstandingPenalties = 700;
  const penaltyCount = 120;
  const outstandingCount = 40;
  return {
    success: true,
    data: {
      ...
      collectionRate: (collectedPenalties / totalPenalties) * 100,
      penaltyBreakdown: {
        lateReturnFees: 1500,
        damageFees: 700,
        lostBookFees: 250,
      ...
      monthlyData: [
        { month: "2024-04", totalPenalties: 800, collectedPenalties: 600, outstandingPenalties: 200 },
        { month: "2024-05", totalPenalties: 900, collectedPenalties: 700, outstandingPenalties: 200 },
        { month: "2024-06", totalPenalties: 800, collectedPenalties: 500, outstandingPenalties: 300 }
      ],
      avgPenaltyAmount: totalPenalties / penaltyCount,
      topPenaltyReasons: [
        { reason: "Late Return", count: 80, totalAmount: 1500 },
        { reason: "Minor Damage", count: 25, totalAmount: 400 }
      ]
    }
  }
}
...
const userStats = reactive({
  ...
  dateRange: { from: '', to: '' },
  data: {
    activeUsers: 0, totalUsers: 0, newRegistrations: 0, libraryVisits: 0,
    avgBorrowsPerUser: 0, avgVisitsPerUser: 0, userDemographics: { students: 0, faculty: 0, staff: 0, external: 0 },
    topActiveUsers: [], activityTrends: [], peakUsageHours: [], peakUsageDays: []
  }
})
const financialStats = reactive({
  ...
  data: {
    ...
    penaltyBreakdown: { lateReturnFees: 0, damageFees: 0, lostBookFees: 0, otherFees: 0 },
    monthlyData: [], avgPenaltyAmount: 0, topPenaltyReasons: []
  }
})
...
const exportCollectionStats = async () => {
  ...
  try {
    ...
    const data = [
      ...
      ['Average Books Per Category', collectionStats.data.averageBooksPerCategory.toFixed(1)],
      [''],
      ['Popular Books'],
      ...
      [''],
      ['Category Breakdown'],
      ['Category', 'Total Books', 'Available', 'Borrows'],
      ...collectionStats.data.categoryStats.map(cat => [cat.name, cat.totalBooks, cat.availableBooks, cat.borrowCount])
    ]
    const sheet = XLSX.utils.aoa_to_sheet(data)
...
const fetchUserStats = async () => {
  ...
  try {
    const response = await getUserActivityStatistics({
      from: userStats.dateRange.from,
      to: userStats.dateRange.to
    })
    if (response.success) userStats.data = response.data
  } catch (error) {
...
const fetchFinancialStats = async () => {
  ...
  try {
    ...
    if (response.success) financialStats.data = response.data
  } catch (error) {
